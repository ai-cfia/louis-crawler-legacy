...
class GoldiePlaywrightSpider(PlaywrightSpider):
    ...
    def __init__(self, max_depth=1, scraped_urls_file="scraped_urls.txt", pending_urls_file="pending_urls.txt", *args, **kwargs):
        """
        ...
        self.logger.info(f"scraped_urls_file={self.scraped_urls_file}, pending_urls_file={self.pending_urls_file}")
    ...
    def _load_scraped_urls(self):
        ...
        if os.path.exists(self.scraped_urls_file):
            try:
                ...
                self.logger.info(f"Loaded {len(self.scraped_urls)} URLs from {self.scraped_urls_file}")
            except Exception as e:
                self.logger.error(f"Error loading scraped URLs: {e}")
                self.scraped_urls = set()
        else:
            self.logger.info(f"No existing scraped URLs file found: {self.scraped_urls_file}")
                    ...
                    for line in f:
                        ...
                        if line:
                            ...
                            if '|' in line:
                                ...
                                self.pending_urls.add((line, 0))
                self.logger.info(f"Loaded {len(self.pending_urls)} pending URLs from {self.pending_urls_file}")
            except Exception as e:
                self.logger.error(f"Error loading pending URLs: {e}")
                self.pending_urls = set()
        else:
            self.logger.info(f"No existing pending URLs file found: {self.pending_urls_file}")
    ...
    def start_requests(self):
        ...
        for url, depth in self.pending_urls.copy():  # Use copy to avoid modification during iteration
        ...
        for url in self.start_urls:
            if not self._is_url_scraped(url) and not any(pending_url == url for pending_url, _ in self.pending_urls):
                ...
                self.logger.info(f"Skipping already scraped or pending start URL: {url}")
    
    def parse(self, response):
        """Parse response and extract data."""
        current_depth = self._get_request_depth(response)
        self.logger.info(f"Parsing with Playwright (depth {current_depth}): {response.url}")
    ...
    def extract_and_follow_urls(self, response, current_depth):
        ...
        if should_follow_links:
            self.logger.info(f"Following links from depth {current_depth} (max_depth={self.max_depth})")
        elif should_save_for_later:
            self.logger.info(f"At max depth {self.max_depth}, saving links for future runs from: {response.url}")
        else:
            self.logger.info(f"Beyond max depth {self.max_depth}, not processing links from: {response.url}")
            return
            ...
            if link and not link.startswith('#') and not link.startswith('mailto:'):
                ...
                if self.allowed_domains:
                    ...
                    if any(allowed in domain for allowed in self.allowed_domains):
                        # Check if URL has already been scraped or is pending
                        if not self._is_url_scraped(absolute_url) and not any(pending_url == absolute_url for pending_url, _ in self.pending_urls):
                            if should_follow_links:
                            ...
                            self.logger.debug(f"Skipping already scraped or pending URL: {absolute_url}")
        
        if should_follow_links:
            self.logger.info(f"Found {links_found} links, following {links_followed} new ones at depth {next_depth}")
        elif should_save_for_later:
            self.logger.info(f"Found {links_found} links, saved {links_saved_for_later} new ones for future runs at depth {next_depth}")
    ...
    def closed(self, reason):
        ...
        if self.pending_urls:
            self.logger.info(f"You can resume scraping later with {len(self.pending_urls)} pending URLs")
            ...
            if os.path.exists(self.pending_urls_file):
                try:
                    ...
                    self.logger.info(f"Removed empty pending URLs file: {self.pending_urls_file}")
                except Exception as e:
